...
export default function ProgressPage() {
  ...
  const sortedTasks = [...dayData.tasks].sort(
        ...
        >
          ...
          >
            <StatCard
              ...
              sub={
                total > 0
                  ? `${Math.round((completedCount / total) * 100)}% done`
                  : "No tasks yet"
              }
            ...
            <StatCard
              label="Day streak"
              value={`${streak}d`}
              sub="consecutive full days"
              accentGlow={ACCENT_COLOR}
            />
            <StatCard
              label="Active days"
              value={`${gStats.activeDays}`}
              sub="days with tasks logged"
              accentGlow={ACCENT_COLOR}
            />
          </motion.div>
          ...
          <motion.div
            variants={fadeUp}
            className="flex lg:hidden rounded-xl border border-white/[0.06] bg-[#0f0f0f] p-1 gap-1"
          >
            {tabs.map((tab) => (
              <button
                key={tab.key}
              ...
              {mobileTab === "tasks" && (
                ...
                >
                  ...
                  <SectionCard
                    title="Tasks"
                    subtitle={
                      total === 0 ? "No tasks yet — add one above" : undefined
                    }
                    accentGlow={ACCENT_COLOR}
                  >
