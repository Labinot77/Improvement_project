...
export function CalendarView({ selected, onSelect, lessons }: Props) {
  ...
  return (
    ...
    >
      <Calendar
        ...
        modifiers={{
          hasLesson: (date) => lessonDates.has(formatDate(date)),
        }}
        modifiersClassNames={{
          today:
            "after:absolute after:bottom-0.5 after:right-0.5 after:w-2 after:h-2 after:bg-yellow-400 after:rounded-full",
          hasLesson:
            "after:absolute after:bottom-0.5 after:right-0.5 after:w-2 after:h-2 after:bg-amber-500 after:rounded-full",
        }}
      />
      ...
      <div className="flex flex-col gap-1.5 mt-4 pt-4 border-t border-white/[0.05]">
        {[
          ...
          { color: "#f59e0b", label: "Lesson logged" },
        ].map(({ color, label }) => (
